...
def start(message):
    if message.text == '/start':
        bot.send_message(message.from_user.id, "Привет, меня зовут Никита, " +
                         "я расскажу о себе при помощи этого телеграм-бота.")
        bot.send_message(message.from_user.id, "Мне 20 лет. Я живу в " +
                         "Краснодаре, преподаю программирование. Раньше я " +
                         "полтора года работал фуллстек разработчиком. Учился" +
                         " я в Анапском Индустриальном Техникуме.")
        bot.send_message(message.from_user.id, "Для дальнейшего " +
                         "взаимодействия со мной напиши в чат /help.")
...
def content(message): 
    ...
    if message.text == '/help':
        bot.send_message(message.from_user.id, "Нажми на одну из кнопок, " +
                         "либо введи одну из следующих команд: ", 
                         reply_markup=markup)
        bot.send_message(message.from_user.id, "/start - запускает бота, " +
                         "отправляет первые сообщения", reply_markup=markup)
        bot.send_message(message.from_user.id, "/help - рассказывает о " +
                         "существующих командах", reply_markup=markup)
        bot.send_message(message.from_user.id, "/nextstep - команда для " +
                         "отправки сообщения настоящему Никите", 
                         reply_markup=markup)
        bot.send_message(message.from_user.id, "/getsourcecode - получить " +
                         "ссылку на исходный код бота", reply_markup=markup)
    ...
    # обработка команды - /getsourcecode
        ...
        bot.send_message(message.from_user.id, 
                         "https://github.com/NikitHome/resume-bot-for-yandex", 
                         reply_markup=markup)
    ...
    # обработка команды (кнопки) - Главное увлечение
        ...
        bot.send_message(message.from_user.id, 'Мое главное увлечение это ' +
                         'видеоигры. Я воспринимаю это скорее не как способ ' +
                         '"убить время", а как нечто близкое к искусству. Это' +
                         ' возможность побывать в таких местах, которых не' +
                         ' существует, или места которые так далеко от меня,' +
                         ' что не представляется возможным побывать там. Это' +
                         ' возможность пережить чью-то историю, чьи-то ' +
                         'чувства и эмоции. Игры дарят возможность ' +
                         'разнообразить свою жизнь до невообразимых масштабов.', 
                         reply_markup=markup)
    ...
    # обработка команды (кнопки) - В чем разница между SQL и NoSQL?
        ...
        bot.send_voice(message.from_user.id, voice_sql_nosql, 
                       reply_markup=markup)
    ...
    # обработка команды (кнопки) - История первой любви
        ...
        bot.send_message(message.from_user.id, 
                         'Хотел бы я научиться говорить как человек...', 
                         reply_markup=markup)
...
def forward_message(message):
    ...
    bot.send_message(message.from_user.id, "Сообщение отправлено Никите.", 
                     reply_markup=markup) 
